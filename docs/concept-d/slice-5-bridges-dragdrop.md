...
| 3 | **Label-prompt sheet on drop** (per JWPK `msg_e1u1mrgk8v`). On drop, a modal sheet opens prefilled with `fileURL.lastPathComponent` as the label; user can edit before tap-Save → upload starts with the chosen label. Cancel dismisses the sheet + abandons the upload. Multi-file drop opens one sheet listing all N files, each with its own label field. (Reversal from `5940b6c` first-cut, which shipped filename-verbatim — needs build follow-up.) | new `Antchat/Views/Chat/AttachmentLabelSheet.swift` triggered from `AttachmentUploadService.handleDrop` before the upload call |
