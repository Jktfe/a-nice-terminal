...
- Future-prevent: probe runs that set `HOME=/tmp/ant-m*-probe.../home` should also set `ANT_TMUX_SOCKET=ant-probe` (or any socket name distinct from the user default). The daemon writes `~/.ant/tmux-<sock>.sh` and uses `tmux -L <sock>` for every call, so probe sessions can't leak into the user's shared tmux server. The Settings → Maintenance "Clean up tmux sessions" button and server boot-time reaper sweep up anything that does still leak.
