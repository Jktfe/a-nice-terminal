...
- macOS PTY cap is `kern.tty.ptmx_max`, default 511 ([[project_pty_exhaustion_diagnostic]]). Per-user, but in practice all jamesking processes share the pool.
...
- Multi-OS-user is irrelevant: fresh-ANT runs as user `jamesking` only. Multi-AGENT (researchant, codex2, evolveantcodex sharing one terminal) is a separate model question — currently agents are *channels* in one chatroom, not separate OS users.
