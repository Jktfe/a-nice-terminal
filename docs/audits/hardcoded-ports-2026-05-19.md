...
Per JWPK: "no skills, tools, hooks, whatever has HARDCODED ports, we should be able to switch them, AND, if the server goes down it should NEVER block their usage". This is the first-pass disk grep across all 6 CLI dot-folders. Each row = a file + line that violates one of those two rules.
