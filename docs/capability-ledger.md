...
| Contract pack registry | contract-pack-v1, 2026-05-23 | CHANGE | Codex | Added `GET /api/contracts` and `GET /api/contracts/:contractId` as the public contract registry. OSS/public contracts can return bodies; premium contracts are listed as stubs only with `body=null`, `bodyAccess=locked`, and `bodyLocation=premium-app-bundle`. Native discovery now advertises `/api/contracts`. Premium contract bodies remain outside the OSS repo/API surface. |
