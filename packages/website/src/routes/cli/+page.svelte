<script lang="ts">
  import { cliCommands, cliCategories, type DocEntry } from '$lib/data/cli-commands';
...
<svelte:head>
  <title>CLI Reference | ANT - A Nice Terminal</title>
  <meta name="description" content="Complete reference for all 15 ANT CLI commands. Manage terminal and conversation sessions from any command line." />
  <meta property="og:title" content="CLI Reference | ANT - A Nice Terminal" />
  <meta property="og:description" content="15 commands for humans and agents to manage terminal and conversation sessions." />
  <meta property="og:url" content="https://antonline.dev/cli" />
</svelte:head>

<div class="mx-auto max-w-4xl px-6 py-16">
  <!-- Header -->
  <div class="mb-12">
    <h1 class="text-4xl font-bold text-white">CLI Reference</h1>
    <p class="mt-3 text-lg text-neutral-400">15 commands for humans and agents</p>
  </div>

  <!-- Installation -->
  <div class="mb-12 rounded-xl border border-white/[0.06] bg-[var(--color-surface)] p-6">
    <h2 class="mb-4 text-lg font-semibold text-white">Installation</h2>
    <div class="flex flex-col gap-3 sm:flex-row sm:gap-6">
      <div class="flex-1">
        <p class="mb-2 text-xs font-medium uppercase tracking-wider text-neutral-500">Global install</p>
        <code class="block rounded-lg bg-black/40 px-4 py-3 font-mono text-sm text-emerald-400">npm install -g a-nice-terminal</code>
      </div>
      <div class="flex-1">
        <p class="mb-2 text-xs font-medium uppercase tracking-wider text-neutral-500">Or run directly</p>
        <code class="block rounded-lg bg-black/40 px-4 py-3 font-mono text-sm text-emerald-400">npx a-nice-terminal</code>
      </div>
    </div>
  </div>
  ...
  <div class="flex flex-col gap-3">
    {#each filtered as cmd (cmd.name)}
      <div class="rounded-xl border border-white/[0.06] bg-[var(--color-surface)] transition hover:border-white/[0.12]">
        ...
        >
          <code class="shrink-0 font-mono text-sm font-medium text-emerald-400">ant {cmd.name}</code>
          ...
          <span class="ml-auto text-sm text-neutral-400 hidden sm:inline">{cmd.description}</span>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-4 w-4 shrink-0 text-neutral-500 transition-transform {expanded.has(cmd.name) ? 'rotate-180' : ''}"
        ...
        <p class="px-5 pb-3 text-sm text-neutral-400 sm:hidden">{cmd.description}</p>

        <!-- Expanded content -->
        {#if expanded.has(cmd.name)}
                ...
                <div class="overflow-hidden rounded-lg border border-white/[0.06]">
                  ...
                  <div class="flex items-center justify-between bg-black/60 px-4 py-2.5">
                    ...
                    <button
                      ...
                      onclick={() => copyExample(cmd.example!, cmd.name)}
                    >
                      {copiedName === cmd.name ? 'Copied!' : 'Copy'}
                    </button>
                  </div>
                  <pre class="overflow-x-auto bg-black/40 px-4 py-3"><code class="font-mono text-sm text-neutral-300"><span class="text-neutral-500">$</span> {cmd.example}</code></pre>
                </div>
              </div>
            {/if}
          </div>
  ...
  {#if filtered.length === 0}
    <div class="rounded-xl border border-white/[0.06] bg-[var(--color-surface)] px-6 py-12 text-center">
      <p class="text-neutral-500">No commands match your search.</p>
    </div>
  {/if}
