<script lang="ts">
  ...
  // text elements from the snapshot, then uses the same claim scorer.
  // Per-block tagging is still a follow-up, but the live lens is present.
    ...
    <section class="univer-shell" aria-label="Univer workspace">
      ...
      {#key artefact.id}
        <UniverViewer
          ...
          contentId={data.content?.id ?? artefact.id}
