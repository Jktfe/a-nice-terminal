 ...
 * mine is a standalone non-destructive Desk verb, not coupled to delete.
 */

import { error, json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';
import { mineDesk, TerminalDeskError } from '$lib/server/terminalDeskFacade';
...
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  try {
    return json(mineDesk({ deskId, actor, reason: body?.reason }));
  } catch (cause) {
