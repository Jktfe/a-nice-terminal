...
function eventFor(search: string) {
  return {
    request: new Request(`http://localhost/api/fs/list${search}`),
...
describe('/api/fs/list', () => {
  ...
  it('GET 400 on relative path', async () => {
    const res = await run(GET as unknown as AnyHandler, eventFor('?path=relative'));
    expect(res.status).toBe(400);
  });

  it('GET 404 for nonexistent path', async () => {
    const res = await run(GET as unknown as AnyHandler, eventFor('?path=/no/such/path'));
  ...
  it('GET respects showHidden', async () => {
    ...
    const resHidden = await run(GET as unknown as AnyHandler, eventFor(`?path=${tmpDir}&showHidden=true`));
    ...
    const resNoHidden = await run(GET as unknown as AnyHandler, eventFor(`?path=${tmpDir}`));
