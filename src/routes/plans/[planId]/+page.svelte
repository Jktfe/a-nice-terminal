...
<script lang="ts">
  ...
  const headlineProgress = $derived.by(() => {
    const milestoneProgress = data.cockpit?.progress.milestones;
    if (data.completion.total === 0 && milestoneProgress && milestoneProgress.total > 0) {
      return {
        label: 'milestones',
    ...
    <div class="layout" class:has-detail={!!selected}>
      <div class="gantt">
        {#if filtered.length === 0}
          <p class="empty">
            {#if data.completion.total === 0 && (data.cockpit?.progress.milestones.total ?? 0) > 0}
              This plan is currently milestone-driven, so there are no task rows for the Gantt view yet. Use the Dashboard for milestone progress.
            {:else}
